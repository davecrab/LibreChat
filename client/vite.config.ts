...
const backendPort =
  (process.env.BACKEND_PORT && Number(process.env.BACKEND_PORT)) || 3080;
...
export default defineConfig(({ command }) => ({
  ...
  plugins: [
    ...
    // Disable PWA during Vercel builds
    !isVercel &&
      VitePWA({
        injectRegister: 'auto',
      ...
      output: {
        manualChunks(id: string) {
          ...
          if (normalizedId.includes('node_modules')) {
            if (normalizedId.includes('@codesandbox/sandpack')) return 'sandpack';
            if (normalizedId.includes('react-virtualized')) return 'virtualization';
            if (
              normalizedId.includes('i18next') ||
              normalizedId.includes('react-i18next')
            )
              return 'i18n';
            if (normalizedId.includes('lodash')) return 'utilities';
            if (normalizedId.includes('date-fns')) return 'date-utils';
            if (normalizedId.includes('@dicebear')) return 'avatars';
            if (
              normalizedId.includes('react-dnd') ||
              normalizedId.includes('react-flip-toolkit')
            )
              return 'react-interactions';
            if (normalizedId.includes('react-hook-form')) return 'forms';
            ...
            if (
              ...
              normalizedId.includes('downshift')
            )
              return 'advanced-inputs';

            if (normalizedId.includes('heic-to')) return 'heic-converter';
            if (normalizedId.includes('@radix-ui')) return 'radix-ui';
            if (normalizedId.includes('framer-motion')) return 'framer-motion';
            if (normalizedId.includes('highlight.js'))
              return 'markdown_highlight';
            if (normalizedId.includes('katex')) return 'math-katex';
            if (normalizedId.includes('hast-util-raw'))
        ...
        assetFileNames: (assetInfo) => {
          if (
            assetInfo.names?.[0] &&
            /\.(woff|woff2|eot|ttf|otf)$/.test(assetInfo.names[0])
          ) {
